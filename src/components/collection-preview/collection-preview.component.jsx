...
const CollectionPreview = ({ collectionType, collections }) => (
  ...
  <div className="collection-preview">
    {(() => {
      ...
      let collectionsPopular = [...collections].sort(
        (a, b) => b.imdbRating - a.imdbRating
      );
      collectionsNowShowing = collectionsNowShowing.filter(
        (collection, idx) => collection.Year < 2013
      );
      collectionsPopular = collectionsPopular.filter(
        (collection, idx) => collection.Year < 2013
      );
      let collectionsUpcoming = [...collections].filter(
        (collection, idx) => collection.Year >= 2013
      );

      if (collectionType === "now-showing-indirect") {
        return collectionsNowShowing
          .filter((collection, idx) => idx < 10)
